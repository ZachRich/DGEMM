...
int main(int argc, char *argv[]) {
    ...
    double *a = (double *)malloc(ROWLEN * ROWLEN * sizeof(double));
    double *b = (double *)malloc(ROWLEN * ROWLEN * sizeof(double));
    double *c = (double *)malloc(ROWLEN * ROWLEN * sizeof(double));
...
//    printf("\n");
    ...
    start = clock(); //Start DGEMM_parallel
     printf("Start DGEMM_Parallel \n");
     DGEMM_parallel(ROWLEN, a, b, c);
     printf("End DGEMM_Parallel \n");
     end = clock();
     cpu_time_used = ((double)(end - start))/CLOCKS_PER_SEC;
     printf("the elapsed CPU time is %lf\n", cpu_time_used);
     printf("Clocks per sec: %d\n", CLOCKS_PER_SEC);
